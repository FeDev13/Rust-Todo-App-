use druid::widget::{Button, Checkbox, Controller, Flex, Label, List, Padding, TextBox, ZStack};
use druid::{Code, Env, Event, EventCtx, Menu, MenuItem, Point, UnitPoint, Widget, WidgetExt};
...
pub fn ui_builder() -> impl Widget<TodoState> {
    let header = Flex::row()
        ...
        .with_child(
            Button::new("Agregar").on_click(|_ctx, data: &mut TodoState, _env| {
            ...
            .with_child(Button::new("...").on_click(
                |ctx: &mut EventCtx, data: &mut TodoItem, _env| {
                    ...
                    let menu: Menu<TodoState> =
                        Menu::empty().entry(MenuItem::new("Borrar").on_activate(
